...
async function getArticleCategories() {
  return db
    .select({
      ...
      articleCount: count(articleInfo.id).as('article_count'),
    })
    .from(articleCategory)
    .leftJoin(articleInfo, and(eq(articleInfo.categoryId, articleCategory.id), eq(articleInfo.deleted, false)))
    .groupBy(articleCategory.id)
    .orderBy(desc(articleCategory.sort))
}

export default defineApiEventHandler('admin.article', async (event) => {
