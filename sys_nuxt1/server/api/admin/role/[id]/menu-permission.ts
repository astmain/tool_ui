 ...
 */
import { getRouterParam, getQuery, readBody, createError } from 'h3'
...
export default defineApiEventHandler('admin.role.menuPermission', async (event) => {
  ...
  if (method === 'PUT') {
    ...
    await db.transaction(async (tx) => {
      await tx.delete(roleMenuPermission).where(eq(roleMenuPermission.roleId, roleId))
      ...
      if (createData.length > 0) {
        await tx.insert(roleMenuPermission).values(createData)
      }
    })
