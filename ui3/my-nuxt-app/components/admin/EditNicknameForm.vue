...
async function handleSubmit() {
  ...
  try {
    const res = await $fetch<any>('/api/auth/profile', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: { role: 'admin', field: 'nickname', nickname: nickname.value.trim(), email: props.userInfo.email, code: code.value.trim() },
    })
    if (res.code === 200) {
      emit('success', { nickname: res.data.nickname })
    } else {
