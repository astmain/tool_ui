<template>
  <div class="p-4 flex flex-col gap-4">
    ...
    <Com1Card>
      <div class="space-y-4">
        <div v-for="item in settingList" :key="item.key" class="flex items-center gap-4">
        ...
        <div class="border-t pt-4 mt-4">
          ...
          <Com1Button text="导出 SQL 备份文件" variant="secondary" size="medium" :loading="backingUp" @click="handleBackup" />
          <p v-if="backupMsg" class="text-sm text-green-600 mt-2">{{ backupMsg }}</p>
        </div>
      </div>
    </Com1Card>
  </div>
...
const settingList = ref<Array<{ key: string; value: string }>>([])
const newKey = ref('')
const newValue = ref('')
...
async function fetchSettings() {
  ...
  if (res.code === 200) {
    settingList.value = Object.entries(res.data).map(([key, value]) => ({ key, value: value as string }))
  }
}
...
async function saveSetting(item: { key: string; value: string }) {
  ...
  if (res.code === 200) {
    const idx = settingList.value.findIndex((s) => s.key === item.key)
    if (idx === -1) settingList.value.push({ key: item.key, value: item.value })
