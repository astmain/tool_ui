 ...
 */
import { defineEventHandler, getQuery, getRouterParam, readBody, createError } from 'h3'
...
export default defineEventHandler(async (event) => {
  ...
  if (method === 'POST') {
    ...
    try {
      const [newUser] = await db
        .insert(user)
        .values({ nickname, email, password: hashed, remark })
        .returning({ id: user.id, nickname: user.nickname, email: user.email, avatar: user.avatar, remark: user.remark })

      for (const rid of roleIds) {
        await db.insert(userRole).values({ userId: newUser.id, roleId: rid })
      }
  ...
  if (method === 'PUT') {
    ...
    if (roleIds !== undefined) {
      ...
      for (const rid of roleIds) {
        await db.insert(userRole).values({ userId: id, roleId: rid })
      }
    }
