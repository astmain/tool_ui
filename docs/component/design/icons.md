...
U1Design 的 Icon 图标用于按钮, 表单, 导航和反馈场景. 图标保持一致的线性风格, 统一尺寸和描边, 不使用 emoji 作为结构图标.
...
4. 按钮图标直接传字符串名称, 例如 <code>icon="add"</code> 或 <code>iconRight="check"</code>.
5. 未匹配到的图标名称会回退到 <code>close</code>, 避免图标区域空白.
