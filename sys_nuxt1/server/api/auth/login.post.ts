...
export default defineApiEventHandler('auth.login', async (event) => {
  ...
  if (isAdmin) {
    ...
    setCookie(event, ADMIN_COOKIE_NAME, token, setCookieOpts)
  } else {
    const token = await signToken({ id: dbUser.id, type: 'user', nickname: dbUser.nickname, roleKeys, roleIds })
    setCookie(event, USER_COOKIE_NAME, token, setCookieOpts)
  }
